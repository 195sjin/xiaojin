...
注/*则能够匹配所有请求，例如在使用过滤器时，若需要对所有请求进行过滤，就需要使用/*的写法
...
即 首先经过web.xml文件，找到了SpringMVC.xml配置文件，通过这个配置文件进行扫描，找到控制器，再将请求地址跟@RequestMapping里面的value属性进行匹配，匹配成功就通过Themeleaf进行页面的转发。
...
```
<h1>测试请求参数</h1>
<a th:href="@{/testServletApi(username='aaa',password=12345)}">测试ServletApi获取请求参数</a><br>
<a th:href="@{/textParam(username='aaa',password=12345)}">测试控制器的形参获取请求参数</a><br>
<form th:action="@{/textParam}" method="post">
...
```
    ...
    public String textParam(String username,String password,String hobby){
...
例：标签里面的属性为user_name,但是在控制器方法中形参为username，一般来说改不掉源代码，就需要使用该注解，在控制器中 @RequestParam（"user_name") String username

@RequestParam注解一共有三个属性：

value：指定为形参赋值的请求参数的参数名

required：设置是否必须传输此请求参数，默认值为true，表示该注解必须要有参数，否则就报错。
...
叭叭叭讲一堆，我也很蒙蔽，在开发中用到配置文件的时候较少，现在都是再用注解进行开发，下面的是他再讲继承这个类为啥能代替web.xml
